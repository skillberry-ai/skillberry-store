...
def test_add_description(vector_index):
    ...
    vector_index.add_description(test_description)
...
def test_search_description(vector_index):
    ...
    for desc in test_descriptions:
        vector_index.add_description(desc)
    ...
    assert all(isinstance(idx, (int, np.int64)) for idx, _ in results)
    assert all(isinstance(dist, (float, np.float32)) for _, dist in results)
...
def test_update_description(vector_index):
    ...
    vector_index.add_description(original_desc)
    original_size = vector_index.index.ntotal

    # Update the description
    vector_index.update_description(updated_desc, 0)
...
def test_delete_description(vector_index):
    ...
    vector_index.delete_description(0)
...
def test_save_and_load_index(temp_index_file):
    ...
    vector_index.add_description(test_desc)
...
def test_index_persistence(temp_index_file):
    ...
    vector_index1.add_description("Test persistence")
