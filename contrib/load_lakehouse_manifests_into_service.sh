...
GENAI_PROJECT_ROOT="~/genai-lakehouse-mapping"
...
for value in "${values[@]}"; do
    ...
    curl -X POST -H 'accept: application/json' -H 'Content-Type: multipart/form-data' -F "file=@$GENAI_TRANSFORMATION_FILE" "http://localhost:8000/manifests/add?file_manifest=${file_manifest_url}" | jq .
