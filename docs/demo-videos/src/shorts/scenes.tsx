      ...
      <div style={{ position: 'absolute', inset: 0, padding: '60px 80px', display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div style={{ flex: 1, display: 'flex', gap: 48 }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 16 }}>
            {[
              { icon: '🔗', t: 'GitHub URL — any Anthropic skills repo', d: 18 },
              { icon: '📦', t: 'ZIP File — for air-gapped environments', d: 32 },
              { icon: '📁', t: 'Local Folder — ideal for CI/CD pipelines', d: 46 },
            ].map((b, i) => (
              <div key={i} style={{ opacity: appear(b.d, 14), display: 'flex', gap: 14, alignItems: 'flex-start', padding: '14px 18px', borderRadius: 10, background: COLORS.bgAlt, border: `1px solid ${COLORS.border}` }}>
    ...
    <AbsoluteFill style={{ background: COLORS.bg, opacity: fo, fontFamily: FONT }}>
      ...
      <div style={{ position: 'absolute', inset: 0, padding: '60px 80px', display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div style={{ flex: 1, display: 'flex', gap: 28, alignItems: 'stretch' }}>
          {[
            { icon: '🔌', title: 'Virtual MCP Servers', desc: 'Expose any skill as a standalone MCP endpoint with its own port and SSE URL. Plug into Claude, Cursor, or any AI client.', color: COLORS.primary, delay: 14 },
            { icon: '🗂️', title: 'Virtual NFS Servers', desc: 'Mount a skill as a read-only WebDAV or NFSv3 filesystem. Claude Code reads files directly — no REST API needed.', color: '#7B5CD8', delay: 30 },
            { icon: '📊', title: 'Observability', desc: 'Live metrics dashboard. Prometheus on :8090, OpenTelemetry traces to Jaeger. Grafana-ready out of the box.', color: COLORS.warning, delay: 46 },
          ].map((c, i) => (
            <div key={i} style={{ flex: 1, opacity: appear(c.delay, 16), transform: `translateY(${interpolate(f, [c.delay, c.delay + 18], [22, 0], clampOpts)}px)`, border: `2px solid ${c.color}`, borderRadius: 14, padding: '28px 28px', background: `${c.color}06`, display: 'flex', flexDirection: 'column', gap: 14 }}>
...
export const TzOutro: React.FC = () => {
  ...
  const items = [
    { icon: '🌐', label: 'Web UI',   val: 'localhost:8002', delay: 34 },
    { icon: '📚', label: 'REST API', val: 'localhost:8000/docs', delay: 46 },
    { icon: '📦', label: 'Install',  val: 'pip install skillberry-store', delay: 58 },
    { icon: '🐳', label: 'Docker',   val: 'make docker-run', delay: 70 },
  ];
  return (
    <AbsoluteFill style={{ background: COLORS.bg, opacity: fi, fontFamily: FONT }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', justifyContent: 'center', maxWidth: 1200 }}>
          {items.map((item, i) => (
            <div key={i} style={{ opacity: appear(item.delay, 14), transform: `translateY(${interpolate(f, [item.delay, item.delay + 16], [18, 0], clampOpts)}px)`, padding: '14px 20px', background: COLORS.bgAlt, borderRadius: 10, border: `1px solid ${COLORS.border}`, display: 'flex', gap: 12, alignItems: 'center' }}>
