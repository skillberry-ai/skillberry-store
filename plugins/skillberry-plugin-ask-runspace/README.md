...
- **MCP servers** (`mcp_servers`): a JSON object of MCP servers to expose to the agent, in
  ...
  The **request box is prefilled with editable store-usage guidance** (and every preset
  includes it) so the agent actually uses that server — creating/updating tools, skills,
  snippets in the store — instead of only writing local files. It's just text: edit it, or
  delete the lines, to change or skip the store guidance for a run.
