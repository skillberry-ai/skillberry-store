...
### Load examples into the service
...
Alternatively, you can download sample tools and JSON data from the `genai-lakehouse-mapping` repository
...
and load them into the service in a similar way as described above

### Engage with the service via OpenAPI (Swagger)

Open a new browser tab/window
Copy `<blueberry-tools-service ip address>:8000/docs` into the browser search bar and press `Enter`

_Note:_ replace with your ipaddress/hostname of the service
...
- Open a new terminal browser tab/window and execute: (to start a local prometheus server)
```bash
...
Traces are available in jaeger: http://localhost:16686


## MCP Server Mode

You can run BTS (blueberry-tools-service) in MCP server mode to allow it to connect to any agent framework that supports MCP

Set the MCP_MODE variable
