...
def initialize_object_handlers() -> None:
    ...
    _initialized = True
    logger.info(f"Initialized {len(_object_handlers)} object handlers: {list(_object_handlers.keys())}")
...
class ObjectHandler:
    ...
    def __init__(self, base_directory: str, object_type: str, use_dict_cache: bool = True):
        """
        ...
        logger.info(f"Initialized ObjectHandler for {object_type} at {base_directory} (dict_cache={'enabled' if use_dict_cache else 'disabled'})")
    ...
    def _initialize_name_cache(self):
        ...
        logger.info(f"Initialized name cache for {self.object_type} with {len(self.name_cache.get_all_names())} names")
    
    def _find_head_uuid(self, objects: List[Tuple[str, Optional[str]]]) -> Optional[str]:
    ...
    def _initialize_dict_cache(self):
        ...
        for entry in os.listdir(self.base_directory):
            ...
            try:
                ...
                logger.warning(f"Could not read {self.object_type} dict for UUID {entry} during cache init: {e}")
        
        logger.info(f"Initialized dict cache for {self.object_type} with {self.dict_cache.size()} dicts")
    ...
    def get_cache_parent_for_head(self, uuid_val: str, name: str) -> Optional[str]:
        ...
        try:
            ...
            logger.debug(f"Object {uuid_val} is already HEAD for '{name}', preserving parent {current_parent}")
        ...
        try:
            ...
            while current_uuid:
                ...
                try:
                    ...
                    logger.warning(f"Could not read object {current_uuid} while fixing chain: {e}")
                    break
    ...
    def lookup_by_name(self, name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(f"Found {self.object_type} with name '{name}' (UUID: {head_uuid})")
            return object_dict
        except Exception as e:
    ...
    def read_dict(self, uuid: str) -> Dict[str, Any]:
        ...
        if self.dict_cache:
            ...
            if cached_dict is not None:
                ...
                logger.debug(f"Cache miss for {self.object_type} UUID {uuid} - object does not exist")
                raise HTTPException(
                    status_code=404,
    ...
    def delete_object(self, uuid: str) -> Dict:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Error deleting {self.object_type}: {str(e)}"
            )
    ...
    def list_all_dicts(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if os.path.exists(self.base_directory):
                for entry in os.listdir(self.base_directory):
                    ...
                    try:
                        ...
                        logger.warning(f"Could not read {self.object_type} dict for UUID {entry}: {e}")
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Error listing {self.object_type}s: {str(e)}"
            )
