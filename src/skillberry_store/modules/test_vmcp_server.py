...
def test_init_with_port(mock_fastmcp, mock_register_tools, mock_register_prompts, mock_start_server):
...
@patch('skillberry_store.modules.vmcp_server.VirtualMcpServer._register_tools')
@patch('skillberry_store.modules.vmcp_server.FastMCP')
def test_init_without_port(mock_fastmcp, mock_register_tools, mock_register_prompts, mock_start_server):
...
@patch('skillberry_store.modules.vmcp_server.VirtualMcpServer._register_tools')
@patch('skillberry_store.modules.vmcp_server.FastMCP')
def test_init_with_unavailable_port(mock_fastmcp, mock_register_tools, mock_register_prompts, mock_start_server):
    ...
    try:
        with pytest.raises(ValueError, match=f"Port {occupied_port} is not available"):
            VirtualMcpServer(
                name="test_server", description="Test Server", port=occupied_port, tools=[], snippets=[]
            )
    finally:
        sock.close()
...
@patch('skillberry_store.modules.vmcp_server.VirtualMcpServer._register_tools')
@patch('skillberry_store.modules.vmcp_server.FastMCP')
def test_to_dict(mock_fastmcp, mock_register_tools, mock_register_prompts, mock_start_server):
    ...
    server = VirtualMcpServer(
        name="test_server", description="Test Server", port=None, tools=[], snippets=["snippet1"]
    )
