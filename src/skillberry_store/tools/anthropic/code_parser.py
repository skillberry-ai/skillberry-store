...
def parse_python_function(
    ...
    if docstring:
        ...
        for line in lines:
            ...
            if (
                "args:" in line_stripped.lower()
                or "parameters:" in line_stripped.lower()
            ):
    ...
    for line in lines:
        ...
        if func_match:
            ...
            if not line.strip() or indent > current_function["start_indent"]:
                ...
                functions.append(
                    {"name": current_function["name"], "code": current_function["code"]}
                )
...
def parse_code_file(
    content: str, file_name: str, file_path: str, skill_name: str
) -> List[ParsedTool]:
    ...
    if ext == "py":
        ...
        if len(functions) == 1:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to parse function '{func['name']}' individually: {e}"
                )
        ...
        if len(functions) == 1:
            func = functions[0]
            description, params, returns = parse_bash_function(
                func["code"], func["name"]
            )
            tags = [
                f"file:{file_path}",
...
def parse_code_files(files: List[Dict[str, str]], skill_name: str) -> Dict[str, Any]:
    ...
    for file in files:
        ...
        if is_code_file(file["name"]):
            try:
                file_tools = parse_code_file(
                    file["content"], file["name"], file["path"], skill_name
                )
