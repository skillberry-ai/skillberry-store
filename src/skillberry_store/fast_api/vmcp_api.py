...
def register_vmcp_api(
    ...
    def start_vmcp_server(uuid_or_name: str, request: Request):
        ...
        try:
            ...
            try:
                ...
                if existing:
                    return {"message": f"VMCP server '{server_name}' is already running.", "port": existing.port}
            except Exception:
                pass
            env_id = _extract_env_id(request)
            tool_uuids, snippet_uuids = service._resolve_skill_uuids(vmcp_data.get("skill_uuid"))
            server = service.server_manager.add_server(
                name=server_name, uuid=server_uuid,
                description=vmcp_data.get("description", ""),
                port=vmcp_data.get("port"),
                tools=tool_uuids, snippets=snippet_uuids, env_id=env_id,
            )
            logger.info(f"VMCP server '{server_name}' started on port {server.port}")
            return {"message": f"VMCP server '{server_name}' started successfully.", "port": server.port}
        except KeyError as e:
            raise HTTPException(status_code=404, detail=str(e))
        except ValueError as e:
            raise HTTPException(status_code=404, detail=str(e))
            ...
            raise HTTPException(status_code=500, detail=f"Error starting vmcp server: {str(e)}")
    ...
    def search_vmcp_servers(
        ...
        if not vmcp_descriptions:
            raise HTTPException(status_code=503, detail="VMCP server search is not available")
        try:
            matched_entities = vmcp_descriptions.search_description(search_term=search_term, k=max_number_of_results)
            filtered = [m for m in matched_entities if float(m.get("similarity_score", 0)) <= similarity_threshold]
            to_filter = []
            for m in filtered:
